...
class ScrapeData:
  ...
  def get_data_from_page(self):
    ...
    for data in country_div:
      ...
      st.write(f"{self.country_name.string}")

      #created columns to display data
      col1, col2, col3 = st.columns(3)
      col1.metric("Capital", f"{self.country_capital.get_text()}", "")
      col2.metric("Population", f"{self.country_population.get_text()}", "")
      col3.metric("Area", f"{self.country_area.get_text()}", "")
